...
def PolynomialInterpolation(Time,Amplitude,Order):
    global InterpolatedTime
    global InterpolatedAmplitude
    ...
    ExtrapolatedAmplitude=np.polyval(ExtrapolatedAmplitude,Time)
    #ExtrapolatedAmplitude=interpolate.interp1d(ClippedTime, ClippedAmplitude, fill_value='extrapolate')
    SignalAx.plot(Time,ExtrapolatedAmplitude,'--')  
...
def polyinterpolate():
    ...
    for i in range(0,len(Dividedamplitude)):
        PolynomialInterpolation(DividedTime[i],Dividedamplitude[i],OrderInt)
